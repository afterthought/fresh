...
use fresh::services::fs::{SlowFsConfig, SlowFileSystem};
use std::sync::Arc;
...
fn test_save_blocks_editor_when_fs_hangs() {
    ...
    let inner: Arc<dyn fresh::model::filesystem::FileSystem + Send + Sync> = Arc::new(StdFileSystem);
    ...
    std::thread::scope(|s| {
        ...
        assert!(result.is_ok(), "write_file should succeed after FS is unblocked");
    });
    ...
    harness.editor_mut().save().expect("save should succeed with FS unblocked");
